...
class BlockMLP(nn.Module):
    ...
    def __init__(self, channels, mlp_depth, drop_prob=0.0):
        ...
        for _ in range(mlp_depth):
            self.convs.append(nn.Conv2d(channels, channels, kernel_size=1, bias=True))
...
class BlockMatmulConv(nn.Module):
    def __init__(self, channels, mlp_depth, drop_prob=0.0) -> None:
        super().__init__()
        self.mlp1 = BlockMLP(channels, mlp_depth, drop_prob)
        self.mlp2 = BlockMLP(channels, mlp_depth, drop_prob)
...
class BlockUpdateLayer(nn.Module):
    ...
    def forward(self, inputs, log_degrees):
        h = self.matmul_conv(inputs, log_degrees) + self.skip(inputs)
        h = self.update(h) + h
        return h
